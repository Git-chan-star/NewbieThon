...
Supabase 회원가입 때 표시 이름만 메타데이터로 보낸다.
...
로그인 직후 `select_role` RPC로 `student` 또는 `employer`를 한 번 선택한다. `admin`은 앱 회원가입으로 만들 수 없다. 구인자는 이어서 `onboard_employer` RPC로 조직과 담당자 정보를 완성한다.
...
- 세션 토큰은 Android Keystore 기반 Expo SecureStore에 크기 제한을 고려해 분할 저장한다.
...
공통: 대회 찾기 · 팀 모집/지원 · 팀 대화 · 업무 결과물 관리
```
